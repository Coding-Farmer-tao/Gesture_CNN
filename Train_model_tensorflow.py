...
trData, teData, trLabel, teLabel = train_test_split(X_data, Y_data, test_size=0.3)  # 测试集占总比的0.3，留出法
# 转换为图片的格式 （batch，height，width，channels）
X = trData.reshape(-1, height, width, channel)
Y = trLabel
print('Y:', Y.shape)
batch_size = 10  # 使用MBGD算法，设定batch_size为10,小批量梯度下降法
...
conv_filter_w1 = tf.get_variable('W', [5, 5, 1, 10],
                                 initializer=tf.contrib.layers.xavier_initializer())  # tf.Variable(tf.random_normal([3, 3, 1, 10]))
conv_filter_b1 = tf.Variable(tf.random_normal([10]))
relu_feature_maps1 = tf.nn.relu(tf.nn.conv2d(tf_X, conv_filter_w1, strides=[1, 2, 2, 1],
                                             padding='SAME') + conv_filter_b1)  # SAME:对原图外围进行无限填充，使得边缘像素能够进行合法运算。
print("conv_out1:", relu_feature_maps1)
# 池化层
max_pool1 = tf.nn.max_pool(relu_feature_maps1, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='SAME')  # 3x3窗口的最大池化
...
print("BN_out:", BN_out)
relu_BN_maps2 = tf.nn.relu(BN_out)  # ReLU

# 池化层
max_pool2 = tf.nn.max_pool(relu_BN_maps2, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='SAME')  # 3x3窗口的最大池化
print("max_pool2", max_pool2)

# 将特征图进行展开
max_pool2_flat = tf.reshape(max_pool2, [-1, 60 * 60 * 5])

# 全连接层
fc_w1 = tf.Variable(tf.random_normal([60 * 60 * 5, 50]))
fc_b1 = tf.Variable(tf.random_normal([50]))
fc_out1 = tf.nn.relu(tf.matmul(max_pool2_flat, fc_w1) + fc_b1)

dropout_keep_prob = 0.8
fc1_drop = tf.nn.dropout(fc_out1, dropout_keep_prob)  # dropout防止过拟合
